...
fun BabyProfileScreen(
    ...
    Scaffold(
        ...
        if (isLandscape) {
            // ── LANDSCAPE: Header left, details right ─────────────────────────
            Row(
                modifier = Modifier
                    .fillMaxSize()
                    .padding(paddingValues)
            ) {
                // Left pane: Avatar + quick actions (fixed, no scroll)
                Column(
                    modifier = Modifier
                        .weight(0.38f)
                        .fillMaxHeight()
                    ...
                    verticalArrangement = Arrangement.spacedBy(dimensions.spacingMedium, Alignment.CenterVertically)
                ) {
                    // Avatar
                    Box(
                        modifier         = Modifier
                ...
                Column(
                    ...
                    Button(
                        ...
                        Icon(Icons.Default.Edit, contentDescription = null, tint = MaterialTheme.colorScheme.onPrimary, modifier = Modifier.size(dimensions.iconMedium))
                        Text(text = stringResource(Res.string.baby_action_edit), color = MaterialTheme.colorScheme.onPrimary, fontWeight = FontWeight.Bold, style = MaterialTheme.typography.labelLarge)
                    }
            ...
            Column(
                ...
                Box(
                    modifier = Modifier
                        .fillMaxWidth()
                        .background(Brush.verticalGradient(listOf(customColors.accentGradientStart, customColors.accentGradientEnd)))
                        .padding(horizontal = dimensions.screenPadding, vertical = dimensions.spacingXLarge)
                ) {
                    Column(
                        modifier            = Modifier.fillMaxWidth(),
                        horizontalAlignment = Alignment.CenterHorizontally
                        ...
                        Box(
                            modifier         = Modifier
                                ...
                                .clip(CircleShape)
                                .background(MaterialTheme.colorScheme.onPrimary.copy(alpha = 0.2f))
                                .border(dimensions.borderWidthMedium, MaterialTheme.colorScheme.onPrimary.copy(alpha = 0.4f), CircleShape),
                            contentAlignment = Alignment.Center
                        ) {
                            Text(if (isFemale) "👧" else "👦", fontSize = (dimensions.avatarLarge.value * 0.45f).sp)
                        }
                        Spacer(Modifier.height(dimensions.spacingMedium))
                        ...
                        Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(dimensions.spacingSmall)) {
                            ...
                            ProfileQuickActionButton("📊", stringResource(Res.string.baby_action_growth_chart), onViewGrowthChart, Modifier.weight(1f))
                        }
                    }
                }
                ...
                Column(
                    modifier = Modifier
                        .fillMaxWidth()
                        .padding(horizontal = dimensions.screenPadding, vertical = dimensions.spacingLarge),
                    verticalArrangement = Arrangement.spacedBy(dimensions.spacingMedium)
                ) {
                    ProfileSectionCard(title = stringResource(Res.string.profile_section_basic_info)) {
                ...
                // Edit button
                Column(modifier = Modifier.fillMaxWidth().padding(horizontal = dimensions.screenPadding)) {
                    Button(
                        onClick  = onEditDetails,
    ...
    TopAppBar(
        ...
        actions = {
            IconButton(onClick = onEdit) {
                Icon(Icons.Default.Edit, contentDescription = stringResource(Res.string.baby_action_edit), tint = MaterialTheme.colorScheme.onBackground)
            }
            IconButton(onClick = onDeleteClick) {
                Icon(Icons.Default.Delete, contentDescription = stringResource(Res.string.profile_delete_child), tint = customColors.warning)
            }
        },
...
@Composable
private fun DeleteBabyConfirmDialog(babyName: String, onConfirm: () -> Unit, onDismiss: () -> Unit) {
    val dimensions   = LocalDimensions.current
    val customColors = MaterialTheme.customColors
    AlertDialog(
        onDismissRequest = onDismiss,
        icon  = { Text("🗑️", style = MaterialTheme.typography.displaySmall) },
        title = { Text(stringResource(Res.string.profile_delete_title), fontWeight = FontWeight.Bold, textAlign = TextAlign.Center, style = MaterialTheme.typography.titleMedium, color = MaterialTheme.colorScheme.onSurface) },
        text  = {
            Column(horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.spacedBy(dimensions.spacingSmall)) {
                Text(stringResource(Res.string.profile_delete_message_1), textAlign = TextAlign.Center, style = MaterialTheme.typography.bodyMedium, color = MaterialTheme.colorScheme.onSurfaceVariant)
                Text("\"$babyName\"", textAlign = TextAlign.Center, style = MaterialTheme.typography.titleSmall, fontWeight = FontWeight.Bold, color = MaterialTheme.colorScheme.onSurface)
                Spacer(Modifier.height(dimensions.borderWidthMedium))
                Box(modifier = Modifier.fillMaxWidth().background(customColors.warning.copy(alpha = 0.1f), RoundedCornerShape(dimensions.cardCornerRadius)).padding(dimensions.spacingSmall)) {
                    Text(stringResource(Res.string.profile_delete_warning), textAlign = TextAlign.Center, style = MaterialTheme.typography.bodySmall, color = customColors.warning)
                }
            }
        },
        confirmButton = {
            Button(onClick = onConfirm, colors = ButtonDefaults.buttonColors(containerColor = MaterialTheme.colorScheme.error)) {
                Text(stringResource(Res.string.profile_delete_child), color = MaterialTheme.colorScheme.onError)
            }
        },
...
private fun ProfileSectionCard(title: String, content: @Composable ColumnScope.() -> Unit) {
    ...
    Card(
        ...
        elevation = CardDefaults.cardElevation(defaultElevation = dimensions.cardElevation, pressedElevation = dimensions.cardTonalElevation6)
    ) {
        Column(modifier = Modifier.padding(dimensions.spacingMedium)) {
            Text(title, style = MaterialTheme.typography.labelLarge, fontWeight = FontWeight.SemiBold, color = customColors.accentGradientStart, modifier = Modifier.padding(start = dimensions.profileSectionLabelStartPadding, bottom = dimensions.spacingSmall))
            content()
        }
    }
}
...
private fun ProfileInfoRow(icon: String, label: String, value: String) {
    val dimensions = LocalDimensions.current
    Row(modifier = Modifier.fillMaxWidth().padding(vertical = dimensions.profileInfoRowVerticalPadding), verticalAlignment = Alignment.CenterVertically) {
        Text(text = icon, fontSize = dimensions.profileInfoIconFontSize, modifier = Modifier.width(dimensions.profileInfoIconWidth))
        Text(text = label, style = MaterialTheme.typography.bodyMedium, color = MaterialTheme.colorScheme.onSurface.copy(alpha = 0.6f), modifier = Modifier.weight(1f))
        Text(text = value, style = MaterialTheme.typography.bodyMedium, fontWeight = FontWeight.SemiBold, color = MaterialTheme.colorScheme.onSurface, textAlign = TextAlign.End, modifier = Modifier.weight(1.4f))
    }
}
...
private fun ProfileInfoDivider() {
    val customColors = MaterialTheme.customColors
    HorizontalDivider(color = customColors.accentGradientStart.copy(alpha = 0.12f), thickness = 0.8.dp, modifier = Modifier.padding(vertical = 1.dp))
}

@Composable
private fun ProfileQuickActionButton(emoji: String, label: String, onClick: () -> Unit, modifier: Modifier = Modifier) {
    val customColors = MaterialTheme.customColors
    val dimensions   = LocalDimensions.current
    Column(
        modifier = modifier
            ...
            .clickable { onClick() }
            .background(customColors.glassOverlay.copy(alpha = 0.22f), RoundedCornerShape(dimensions.buttonCornerRadius))
            .border(dimensions.borderWidthThin, customColors.glassOverlay.copy(alpha = 0.45f), RoundedCornerShape(dimensions.buttonCornerRadius))
            .padding(horizontal = dimensions.spacingSmall, vertical = dimensions.profileSectionCardVertPad),
        horizontalAlignment = Alignment.CenterHorizontally,
