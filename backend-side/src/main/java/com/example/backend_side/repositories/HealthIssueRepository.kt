...
interface HealthIssueRepository : JpaRepository<HealthIssue, String> {
    ...
    @Query("SELECT hi FROM HealthIssue hi WHERE hi.baby.babyId = :babyId AND hi.isResolved = false AND hi.severity = :severity")
    fun findUnresolvedBySeverity(@Param("babyId") babyId: String, @Param("severity") severity: Severity): List<HealthIssue>
    ...
    @Query("SELECT hi FROM HealthIssue hi WHERE LOWER(hi.title) LIKE LOWER(CONCAT('%', :searchTerm, '%')) OR LOWER(hi.description) LIKE LOWER(CONCAT('%', :searchTerm, '%'))")
