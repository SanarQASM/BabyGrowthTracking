...
class BenchHolidayController(private val holidayService: BenchHolidayService) {
    ...
    fun addHoliday(@Valid @RequestBody request: BenchHolidayCreateRequest): ResponseEntity<ApiResponse<BenchHolidayResponse>> =
        ResponseEntity.status(HttpStatus.CREATED).body(ApiResponse(true, "Holiday added", holidayService.addHoliday(request)))

    @GetMapping("/national")
    @Operation(summary = "Get all national holidays")
    fun getNationalHolidays(): ResponseEntity<ApiResponse<List<BenchHolidayResponse>>> =
        ResponseEntity.ok(ApiResponse(true, "National holidays retrieved", holidayService.getNationalHolidays()))

    @GetMapping("/bench/{benchId}")
    @Operation(summary = "Get holidays for a specific bench")
    fun getHolidaysForBench(@PathVariable benchId: String): ResponseEntity<ApiResponse<List<BenchHolidayResponse>>> =
        ResponseEntity.ok(ApiResponse(true, "Bench holidays retrieved", holidayService.getHolidaysForBench(benchId)))

    @DeleteMapping("/{holidayId}")
    @Operation(summary = "Delete a holiday — Admin only")
    fun deleteHoliday(@PathVariable holidayId: String): ResponseEntity<ApiResponse<Nothing>> {
